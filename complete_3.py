...
def update_exam():
    ...
    field = input("Enter field to update (exam_name, max_written, max_practical): ")
    value = input("Enter new value: ").upper()
    if field in ("max_written", "max_practical"):
        value = float(value)
...
def generate_report(student_id):
    ...
    CUR.execute("""
        SELECT sub.subject_name, e.exam_id, e.exam_name,
        ...
        WHERE m.id=%s
        ORDER BY sub.subject_name, e.exam_id
    """, (student_id,))
    rows = CUR.fetchall()
    if not rows:
    ...
    # Build DataFrame
    df = pd.DataFrame(rows, columns=["subject","exam_id","exam","written","practical","total","max_total"])
    df["percent"] = (df["total"]/df["max_total"]*100).round(2)

    # Pivot: subjects as rows, exams as column groups 
    pivot = df.pivot(index="subject", columns="exam_id", values=["written","practical","percent"])
    ...
    for ex in exams:
        for sub in subjects:
            val = df[(df["subject"]==sub) & (df["exam_id"]==ex)]["percent"]
...
def predict_final_exam(student_id):
    ...
    CUR.execute("""
        SELECT m.subject_id, e.exam_id, m.total/(e.max_written+e.max_practical)*100
        FROM marks m
        JOIN exams e ON m.exam_id = e.exam_id
        WHERE m.id=%s AND e.exam_id IN (1,2,3)
        ORDER BY m.subject_id, e.exam_id
    """,(student_id,))
    rows = CUR.fetchall()
    if not rows:
    ...
    for exam_id in (1,2,3):
        for sid,_ in subject_rows:
            val = [perc for subj,ex,perc in rows if subj==sid and ex==exam_id]
            features.append(val[0] if val else 0)

    # Predict
    pred = model.predict([features])[0]

    print("Predicted Exam4 (Final) Percentage %:")
